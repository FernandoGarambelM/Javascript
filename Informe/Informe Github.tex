...
\section{Página de html que llama a un archivo externo}
	...
	\end{itemize}  
\section{Ejercicios de W3School}
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.0\textwidth,keepaspectratio]{img/EjerciciosW3school.PNG}
		%\includesvg{img/automata.svg}
		%\label{img:mot2}
		%\caption{Product backlog.}
	\end{figure}
...
console.log("Hoy es " + diaSemanaActual);
	...
	\subsection{Ejercicio 5}
		\lstinputlisting[language=html, caption={Código de html},numbers=left,]{src/ejercicio5.html}
	\begin{itemize}
			\item A continuación una foto de la página
		\end{itemize} 
		\begin{figure}[H]
			\centering
			\includegraphics[width=1.0\textwidth,keepaspectratio]{img/ejercicio5.PNG}
			%\includesvg{img/automata.svg}
			%\label{img:mot2}
			%\caption{Product backlog.}
		\end{figure}
		\begin{itemize}
			\item En esta página web, el usuario ingresa la cantidad de valores que desea en la tabla y al presionar el botón Crear Tabla, se generan valores aleatorios y se muestra una tabla con esos valores. Luego, al presionar el botón Calcular Suma, calculamos la suma de todos los valores en la tabla y la mostramos en la página.
		\end{itemize} 
\section{Referencias}
\begin{itemize}			
